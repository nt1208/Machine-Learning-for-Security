...
y_train = np.load("/mnt/c/DeepVulSeeker/cwe_data/cwe758/train_y.npy")
x_train_emb = np.load("/mnt/c/DeepVulSeeker/cwe_data/cwe758/emb_train.npy")
x_train_ast = np.load("/mnt/c/DeepVulSeeker/cwe_data/cwe758/ast_train.npy")
x_train_dfg = np.load("/mnt/c/DeepVulSeeker/cwe_data/cwe758/dfg_train.npy")
x_train_cfg = np.load("/mnt/c/DeepVulSeeker/cwe_data/cwe758/cfg_train.npy")

# 加载验证集
# val_dataset = pd.read_pickle('data/embedding/val_0')
y_val = np.load("/mnt/c/DeepVulSeeker/cwe_data/cwe758/val_y.npy")
x_val_emb = np.load("/mnt/c/DeepVulSeeker/cwe_data/cwe758/emb_val.npy")
x_val_ast = np.load("/mnt/c/DeepVulSeeker/cwe_data/cwe758/ast_val.npy")
x_val_dfg = np.load("/mnt/c/DeepVulSeeker/cwe_data/cwe758/dfg_val.npy")
x_val_cfg = np.load("/mnt/c/DeepVulSeeker/cwe_data/cwe758/cfg_val.npy")

# 加载测试集
# test_dataset = pd.read_pickle('data/embedding/test_0')
y_test = np.load("/mnt/c/DeepVulSeeker/cwe_data/cwe758/test_y.npy")
x_test_emb = np.load("/mnt/c/DeepVulSeeker/cwe_data/cwe758/emb_test.npy")
x_test_ast = np.load("/mnt/c/DeepVulSeeker/cwe_data/cwe758/ast_test.npy")
x_test_dfg = np.load("/mnt/c/DeepVulSeeker/cwe_data/cwe758/dfg_test.npy")
x_test_cfg = np.load("/mnt/c/DeepVulSeeker/cwe_data/cwe758/cfg_test.npy")
...
# 定义图注意力层
att_ast = MyMultiHeadAttention(200, 2)(input_ast)  # 输出维度，多头数量S
print(att_ast.shape)
...
model.compile(optimizer=Adam(learning_rate=8e-7), loss=["binary_crossentropy"],  # 8e-7
              metrics=["binary_accuracy", tf.keras.metrics.Precision(), tf.keras.metrics.Recall(), tf.keras.metrics.AUC()])
